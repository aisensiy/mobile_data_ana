...
class LeTestCase(unittest.TestCase):
    def setUp(self):
...
31952300,2,,"4118","28495","116.353472","39.981827","QQ","社交沟通","SAFARI浏览器","网页浏览","20131201083955",+0000000000000005.0000,+0000000000000008.0000,+0000000000000622.0000,+0000000000004743.0000,"腾讯头像网","其他","即时聊天类","题材","即时聊天公共网站资源","http://q2.qlogo.cn/g?b=mqq&k=RfJcBw2OQbOWF6vUeHarUQ&t=1382694255&refer=mqq&s=140"
        ...
        self.fileobj = StringIO.StringIO(filedata)

    def test_prepare_user_log(self):
        rows = le.prepare_user_log(self.fileobj)
        self.assertEqual(20, len(rows))
        self.assertEqual({'start_time': '20131201082834', 'location': '116.35347 39.98183'}, rows[0])

    def test_location_extract(self):
        rows = [
            {'start_time': '20131201082834', 'location': '116.35347 39.98183'},
            {'start_time': '20131201082934', 'location': '116.35347 39.98183'},
            {'start_time': '20131201083134', 'location': '116.35347 39.98183'},
            {'start_time': '20131201083234', 'location': '116.35347 39.98183'},
            {'start_time': '20131201083434', 'location': '116.35347 39.98193'},
            {'start_time': '20131201083834', 'location': '116.35347 39.98183'}
        ]

        expected = [
            ('20131201082834', '116.35347 39.98183'),
            ('20131201083134', '116.35347 39.98183'),
            ('20131201083434', '116.35347 39.98193'),
            ('20131201083834', '116.35347 39.98183')
        ]

        self.assertEqual(expected, le.extract_location(rows, split_by_5_minute))
    ...
    def test_save_to_csv(self):
        ...
        le.save_to_csv(fileobj, uid, logs)
        self.assertEqual('001998,20131201082834,116.35347 39.98183\r\n001998,20131201083134,116.35347 39.98183\r\n001998,20131201083434,116.35347 39.98193\r\n001998,20131201083834,116.35347 39.98183\r\n', fileobj.getvalue())
